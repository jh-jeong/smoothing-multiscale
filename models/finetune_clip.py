...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int,
                    ...
                    freeze_conv1: bool = True, nb_classes: int = 1000):
        super().__init__()
        ...
        self.head = nn.Linear(width, nb_classes)
...
class EmbVisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int,
                 heads: int, output_dim: int, checkpoint: bool, dropout: float = 0, emb_dropout: float = 0,
                 drop_path_rate: float = 0, mlp_width: int = 0,
                 freeze_conv1: bool = True, nb_classes: int = 1000, text_embedding: str = ''):
        super().__init__()
        self.input_resolution = input_resolution
        self.freeze_conv1 = freeze_conv1
        self.nb_classes = nb_classes
        ...
        self.register_buffer('text_features', scale * torch.randn(output_dim, nb_classes))
        if text_embedding:
            text_features = torch.load(text_embedding, map_location='cpu')
            assert output_dim == text_features.size(0)
            assert nb_classes == text_features.size(1)
            self.text_features.copy_(text_features)
            print(f"*** Text embedding loaded from {text_embedding} ***")
    ...
    def forward(self, x: torch.Tensor, return_feature=False, return_output=False):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        # shape = [*, width, grid ** 2]
        x = x.reshape(x.shape[0], x.shape[1], -1)
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        ...
        output = 100. * x @ self.text_features
        if return_feature and return_output:
            return output, x
        if return_feature:
            return x
        return output
...
class DCTVisualTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int,
                 heads: int, output_dim: int, checkpoint: bool, dropout: float = 0, emb_dropout: float = 0,
                 drop_path_rate: float = 0, mlp_width: int = 0,
                 freeze_conv1: bool = True, nb_classes: int = 1000, text_embedding: str = ''):
        ...
        DCT_STATS_PATH = f"OUTPUT/dct_stats/imagenet_train_inception_dct2d_ortho_p{patch_size}.npz"
        dct_stats = np.load(DCT_STATS_PATH)
        dct_mean = torch.as_tensor(dct_stats['mean']).reshape(3, 1, 1, patch_size, patch_size)
        dct_var = torch.as_tensor(dct_stats['var']).reshape(3, 1, 1, patch_size, patch_size)
        self.register_buffer('_dct_mean', dct_mean)
        self.register_buffer('_dct_var', dct_var)
    ...
    def forward(self, x: torch.Tensor, return_feature=False, return_output=False):
        x = rearrange(x, 'b c (h p1) (w p2)-> b c h w p1 p2', p1=16, p2=16)
        x = (dct_2d(x) - self._dct_mean) * (self._dct_var + 1e-8).rsqrt()
...
def CLIP_B16(pretrained=False, **kwargs):
    ...
    default_kwargs = {
        ...
        'width': vision_width,
        'checkpoint': False,
        'nb_classes': kwargs.get('num_classes', 1000)
    }
...
@register_model
def CLIP_B16_E(pretrained=False, **kwargs):
    vision_width = 768
    vision_layers = 12
    vision_heads = vision_width // 64
    print (kwargs)
    ...
    model = EmbVisualTransformer(**default_kwargs)
    model.train()
    return model


@register_model
def CLIP_B16_DCT(pretrained=False, **kwargs):
    vision_width = 768
    vision_layers = 12
    vision_heads = vision_width // 64
    print (kwargs)
    ...
    model = DCTVisualTransformer(**default_kwargs)
    model.train()
    return model